...
#define   FINAL_TIME		(double)(				130.0 )
...
//    ============================gimbal bias================================
    ...
    do
    {
        ...
        if(count%100 == 0){
            if (Vin_data > 5.) direction = -1.;
            if (Vin_data < 0.) direction = 1.;
            Vin_data  += direction * step;
        }

        Vcmd[1] = vc2vs_gyro(Vin_data);
    ...
    sprintf(OutFileName, "../LAB/static_motor/dead_zone/%2.3f", Vin_data);

    pFile = fopen(strcat(OutFileName, "_data.out"), "w+t");

    for (idx = 0; idx < N_STEP - 1; idx++)
    {
